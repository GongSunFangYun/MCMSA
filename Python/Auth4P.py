...
class MinecraftAuthenticator:
    ...
    def authenticate(self):
        print("Minecraft Authentication - Device Code Flow\n")

        try:
            print("[1/6] Requesting device code...")
            device_code_response = self.request_device_code()

            print(f"\nVisit this URL: {device_code_response['verification_uri']}")
            print(f"Enter this code: {device_code_response['user_code']}")
            print("Waiting for authentication...")

            token_response = self.poll_for_token(device_code_response['device_code'], device_code_response['interval'])
            print("\n[2/6] Polling for token...")

            xbl_data = self.authenticate_with_xbox_live(token_response['access_token'])
            xbl_token = xbl_data['Token']
            user_hash = xbl_data['DisplayClaims']['xui'][0]['uhs']
            print("[3/6] Xbox Live authentication...")

            xsts_data = self.authenticate_with_xsts(xbl_token)
            xsts_token = xsts_data['Token']
            print("[4/6] XSTS authentication...")

            mc_token_data = self.login_to_minecraft(user_hash, xsts_token)
            mc_access_token = mc_token_data['access_token']
            print("[5/6] Minecraft login...")

            profile = self.get_minecraft_profile(mc_access_token)
            print("[6/6] Getting profile...")
    ...
    def authenticate_with_xbox_live(self, access_token):
        attempts = [f"d={access_token}", access_token]
    ...
    def authenticate_with_xsts(self, xbl_token):
        request_body = {
            "Properties": {
                "SandboxId": "RETAIL",
    ...
    def login_to_minecraft(self, user_hash, xsts_token):
        request_body = {
    ...
    def get_minecraft_profile(self, mc_access_token):
        headers = {
...
def main():
    try:
        ...
        print("\nDone!")
        print("You can use the following sample code to retrieve any field from the returned JSON:\n")
        print("authenticator = MinecraftAuthenticator()")
        print("auth_result = authenticator.authenticate()")
        print("access_token = auth_result['tokens']['minecraft_access_token']")
        print("print(access_token)\n")
        print("Below is the JSON returned from your recent login operation:\n")
